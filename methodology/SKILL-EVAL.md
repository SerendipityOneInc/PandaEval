...
You've been asked to evaluate one or more skills using this methodology. Here's what to do:

1. **Read this entire document** to understand the evaluation framework
2. **For each skill:** follow Phases 1-7 (Pre-flight → Test Design → Execute → Grade → Benchmark → Skill Card → Leaderboard)
3. **After all skills:** run Phase 8-9 (Learning) to update the knowledge base
4. **If a skill scores < 7:** run Phases 10-12 (Improve → Re-eval → Learn)

**Inputs:** A skill path (local directory with SKILL.md), or a skill name to download from ClawHub.
**Outputs:** A score card (markdown) per skill, and optionally an HTML leaderboard.

Read the `knowledge/` directory alongside this file -- it contains accumulated evaluation wisdom from 123 prior evaluations.
...
PandaEval/
  ...
  results/
    skill-cards/                 -- output: one .md per evaluation (446 cards)
    leaderboard/
  ...
  scripts/
    ...
    llm_judge_prompt.md          -- LLM judge prompt template
```
